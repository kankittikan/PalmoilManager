...
public interface TransactionRepository extends JpaRepository<Transaction, Integer> {
    ...
    @Query("select count(*) from Transaction where description.name = :name and date_format(dateAdded, '%dd-%mm-%YYYY') = :date and idTransaction like :id%")
    int findSameDayTransaction(@Param("name") String name, @Param("id") String id, @Param("date") String date);
}
