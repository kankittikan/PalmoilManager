...
import ku.cs.palmoilmnger.model.*;
...
public class ManageRoundController {
    ...
    @GetMapping("/menu/round/manageRound/palm/calculate/{round}")
    public String calculateRound(@PathVariable String round, Model model) {
        WorkRound workRound = workRoundService.findById(round);
        RoundDTO roundDTO = workRoundService.transformToRoundDTO(workRound.getIdWorkRound());
    ...
    @PostMapping("/menu/round/manageRound/palm/calculate/{round}")
    public String calRate(@ModelAttribute CalculateRateDTO calculateRateDTO, @PathVariable String round, RedirectAttributes redirectAttributes) {
        try {
            transactionService.calRate(calculateRateDTO, workRoundService.findById(round));
        } catch (TransactionException e) {
            redirectAttributes.addAttribute("notice", e.getMessage());
            return "redirect:/manager/menu/round/manageRound/palm/calculate/" + round;
        }
